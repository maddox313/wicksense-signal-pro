...
export async function POST(req: NextRequest) {
  const { strategyIds, style, timeframe } = (await req.json()) as {
    strategyIds: string[];
    style: TradingStyle;
    timeframe?: string;
  };
  ...
  const data = await Promise.all(
    WATCHLIST.map(async (symbol) => {
      const { bars } = await fetchBars(symbol, tf, 100);
      const changePercent =
        bars.length >= 2
  ...
  const results = scanMarket(withBars, strategyIds, style, undefined, tf);
